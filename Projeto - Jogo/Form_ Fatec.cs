...
{
    public partial class Form_Fatec : Form
    {
        ...
        private void button_sair_Click(object sender, EventArgs e)
        {
            ...
            this.Close(); // fecha o formulário Fatec
        }

        private void pictureBox1_Click(object sender, EventArgs e)
        {
            string dados_conexao = "Server=127.0.0.1; Database=carteirinhas_estudantes;" +
                " Uid=root; Pwd= ;"; // String de conexão com o banco de dados

//------------------------------------------------------------CONSULTA DE DADOS----------------------------------------------------------------------------------------
            using (MySqlConnection connexao = new MySqlConnection(dados_conexao))
            {
                ...
                "Validade_carteirinha FROM cateirinhas_estudante WHERE id = @id";
                // Consulta SQL para obter informações do estudante com base no ID

                MySqlCommand comandoSQL = new MySqlCommand(informacoes, connexao);
                comandoSQL.Parameters.AddWithValue("@id", id); // Define o valor do parâmetro @id

                connexao.Open(); // Abre a conexão com o banco de dados
...
//------------------------------------------------------------BUSCA E MOSTRAGEM DE RESULTADOS--------------------------------------------------------------------------                    
                    if (Resultados.Read()) // Se a busca retornar resultados
                    {
...
//------------------------------------------------------------QR CODE-----------------------------------------------------------------------------------------------------
                        ...
                        //------------------------------------------------------------MOSTRA(OU NÃO) A FOTO DO ESTUDANTE-------------------------------------------------------------
                        try // Se o banco de dados retornar uma imagem 
                        {
                            ...
                            using (MemoryStream Foto = new MemoryStream(Dados_da_foto)) // Cria um MemoryStream com os bytes da imagem
                            {
                                    ...
                                    pictureBox1.Image = System.Drawing.Image.FromStream(Foto); // Define e carrega a imagem do pictureBox a partir do MemoryStream
                            }
                        }
                ...
                connexao.Close(); // Fecha a conexão com o banco de dados
            }
        }
...
//------------------------------------------------------------CLIQUE PARA A FOTO APARECER--------------------------------------------------------------
        ...
        {
            timer_foto.Stop(); // Pausa o timer_foto
            pictureBox1_Click(sender, e); // Chama o método pictureBox1_Click            
        }
